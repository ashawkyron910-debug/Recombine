class UpdateNodes {
    ...
    writeUpdateItems5(writer) {
        ...
        for (var i = 0; i < this.updNodes.length; i++) {
            ...
            writer.writeUInt8(color.g >>> 0); // Color G
            writer.writeUInt8(color.b >>> 0); // Color B
            var animations = node.owner && node.useAnimations();
            writer.writeUInt8(animations >>> 0); // Animations
            var flags = 0;
            if (node.isVirus)
                flags |= 0x01; // isVirus
            if (node.isAgitated)
            ...
            const nameColor = node.owner && node.owner.nameColor || { r: 0xff, g: 0xff, b: 0xff };
            writer.writeUInt8(nameColor.r >>> 0);
            writer.writeUInt8(nameColor.g >>> 0);
            writer.writeUInt8(nameColor.b >>> 0);
        }
        for (var i = 0; i < this.addNodes.length; i++) {
            var node = this.addNodes[i];
            if (node.nodeId == 0)
