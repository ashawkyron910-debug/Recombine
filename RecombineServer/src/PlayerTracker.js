...
class PlayerTracker {
    constructor(server, socket) {
        ...
        this.nameColor = { r: 0xff, g: 0xff, b: 0xff };
        this.viewNodes = [];
        this.clientNodes = [];
        this.cells = [];
        this.closestCell = null; // Cell closest to the mouse
        ...
        this.isMuted = false;
        // Powerups
        // bigger tolerance - bigger limit!!!
        this.lastRecombine = 0;
        this.lastRecombineDiff = 0;
        this.warningsRecombine = 0;
        this.lastVirus = 0;
        this.lastVirusDiff = 0;
        this.warningsVirus = 0;
        this.lastPush = 0;
        this.lastPushDiff = 0;
        this.warningsPush = 0;
        // Custom commands
    ...
    canUseRecombine(now = Date.now()) {
        const diff = now - this.lastRecombine;
        const delay = this.server.config.powerupRecombineDelay * 1000;
        const tolerance = this.server.config.powerupRecombineTolerance * Math.pow(diff, 0.8) / 2;
        const warningsLimit = this.server.config.powerupRecombineWarningsLimit;
        const every = this.server.config.powerupRecombineEvery;
        const lastDiff = this.lastRecombineDiff;
        const verdict = diff > delay;
        if (diff - tolerance <= lastDiff && lastDiff <= diff + tolerance) {
            this.warningsRecombine += 1;
        } else {
            this.warningsRecombine = 0;
            this.lastRecombineDiff = diff;
        }
        if (this.warningsRecombine >= warningsLimit) this.socket.close();
        if (verdict || every) this.lastRecombine = now;
        return verdict;
    }
    canUseVirus(now = Date.now()) {
        const diff = now - this.lastVirus;
        const delay = this.server.config.powerupVirusDelay * 1000;
        const tolerance = this.server.config.powerupVirusTolerance * Math.pow(diff, 0.8) / 2;
        const warningsLimit = this.server.config.powerupVirusWarningsLimit;
        const every = this.server.config.powerupVirusEvery;
        const lastDiff = this.lastVirusDiff;
        const verdict = diff > delay;
        if (diff - tolerance <= lastDiff && lastDiff <= diff + tolerance) {
            this.warningsVirus += 1;
        } else {
            this.warningsVirus = 0;
            this.lastVirusDiff = diff;
        }
        if (this.warningsVirus >= warningsLimit) this.socket.close();
        if (verdict || every) this.lastVirus = now;
        return verdict;
    }
    canUsePush(now = Date.now()) {
        const diff = now - this.lastPush;
        const delay = this.server.config.powerupPushDelay * 1000;
        const tolerance = this.server.config.powerupPushTolerance * Math.pow(diff, 0.8) / 2;
        const warningsLimit = this.server.config.powerupPushWarningsLimit;
        const every = this.server.config.powerupPushEvery;
        const lastDiff = this.lastPushDiff;
        const verdict = diff > delay && !this.mergeOverride;
        if (diff - tolerance <= lastDiff && lastDiff <= diff + tolerance) {
            this.warningsPush += 1;
        } else {
            this.warningsPush = 0;
            this.lastPushDiff = diff;
        }
        if (this.warningsPush >= warningsLimit) this.socket.close();
        if (verdict || every) this.lastPush = now;
        return verdict;
    }
    forceName(name) {
        if (name) {
            if (!this.originalName) this.originalName = this._name;
            this.setName(name, true);
        }
        else {
            this.setName(this.originalName || this._name, true);
            this.originalName = null;
        }
    }
    setName(name, forced) {
        if (!forced && this.originalName) return;
        this._name = name;
    ...
    forceNameColor(color) {
        if (color) {
            if (!this.originalNameColor) this.originalNameColor = this.nameColor;
            this.setNameColor(color);
        }
        else {
            this.setNameColor(this.originalNameColor || '#FFFFFF');
            this.originalNameColor = null;
        }
    }
    setNameColor(color) {
        if (typeof color === 'string') {
            if (!color.match(/^#?[0-9A-F+]{6}$/i)) return false;
            return this.nameColor = Colors.hexToRgb(color);
        }
    ...
    sendUpdate() {
        ...
        for (; newIndex < this.viewNodes.length && oldIndex < this.clientNodes.length;) {
            ...
            if (node.isMoving || node.type == 0 || node.type == 2 || node.type === 4 || this.server.config.serverGamemode == 3 && node.type == 1)
                updNodes.push(node);
            newIndex++;
            oldIndex++;
        }
        ...
        packetHandler.sendPacket(new Packet.PlayerStats(this));
    }
    updateSpecView(len) {
    ...
    pressV() {
        if (this.spectate || !this.server.run)
            return;
        for (let i = 0; i < this.server.config.ejectMulti; i++) {
            this.server.ejectMass(this, true);
        }
    }
    pressQ() {
        if (this.spectate) {
    ...
    useRecombine(now = Date.now()) {
        if (!this.server.config.powerupRecombine || !this.canUseRecombine(now)) return;
        if (this.cells.length > 1) this.mergeOverride = true;
    }
    useVirus(now = Date.now()) {
        if (!this.server.config.powerupVirus || !this.canUseVirus(now) || this.cells.length < 1) return;
        this.server.spawnVirus(this.mouse, true);
    }
    useFreeze(now = Date.now()) {
        this.cells.forEach((cell) => {
            cell.setBoost(0, 0);
        });
        this.frozen = !this.frozen;
    }
    usePush(now = Date.now()) {
        if (!this.server.config.powerupPush || !this.canUsePush(now)) return;
        if (this.cells.length) this.server.spawnPushes(this.cells[0]);
    }
    getSpecTarget() {
